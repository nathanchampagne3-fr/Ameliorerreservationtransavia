...
export default function ChoixPlace() {
  ...
  const { total, numPassengers, setSelectedSeats } = useBooking();
  ...
  const handleSelect = useCallback((seat: Seat) => {
    ...
    setSeats(prev => {
      const next = prev.map(s => {
        if (s.id === seat.id) return { ...s, status: (s.status === 'selected' ? 'available' : 'selected') as SeatStatus };
  ...
  const selected = seats.find(s => s.status === 'selected');
  const seatTotal = selected ? total + selected.price : total;
  const anySelected = seats.some(s => s.status === 'selected');
              ...
              <div className="flex items-center justify-between px-4 py-2.5">
                <div className="flex items-center gap-3">
                  ...
                  <div className="shrink-0">
                    <SeatSVG type={selected.type} occupied={false} selected={true} />
                  </div>
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 700, color: 'white', lineHeight: '18px' }}>
                      {numPassengers > 1 ? 'Nathan CHAMPAGNE' : 'Marc BORDENET'}
                    </p>
                    <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)', lineHeight: '16px' }}>
                      {typeLabel(selected.type)} · Siège {selected.id}
                    </p>
                  </div>
                </div>
